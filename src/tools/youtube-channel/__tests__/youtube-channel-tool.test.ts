...
import { YoutubeChannelTool } from '../youtube-channel-tool';
...
describe('YoutubeChannelTool', () => {
  ...
  beforeEach(() => {
    ...
    tool = new YoutubeChannelTool();
    tool.register({ server, sapiClient, auth });
  });

  it('registers a tool named "youtube_channel"', () => {
  ...
  it('passes youtube_channel target and parse: true to the scraper', async () => {
    sapiClient.scrape = jest.fn().mockResolvedValue({ data: {} });

    await registeredHandler({ query: '@decodo_official' });
  ...
  it('propagates scraper errors', async () => {
    ...
    await expect(registeredHandler({ query: '@decodo_official' })).rejects.toThrow(
