...
import { TargetSearchTool } from '../target-search-tool';
...
describe('TargetSearchTool', () => {
  ...
  beforeEach(() => {
    ...
    tool = new TargetSearchTool();
    tool.register({ server, sapiClient, auth });
  });

  it('registers a tool named "target_search"', () => {
  ...
  it('returns a text content block with JSON stringified data', async () => {
    const mockData = { results: [{ title: 'Laptop Stand', price: '$29.99' }] };
    sapiClient.scrape = jest.fn().mockResolvedValue({ data: mockData });

    const result = (await registeredHandler({ query: 'laptop stand' })) as {
  ...
  it('passes target_search target and markdown: true to the scraper', async () => {
    sapiClient.scrape = jest.fn().mockResolvedValue({ data: {} });

    await registeredHandler({ query: 'shoes' });
...
describe('TargetSearchTool.transformResponse', () => {
  it('removes all high-char-count fields', () => {
    const tool = new TargetSearchTool();
    const input = {
      results: [{ title: 'Item' }],
      suggested: 'remove me',
