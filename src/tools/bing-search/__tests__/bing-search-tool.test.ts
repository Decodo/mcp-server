...
import { BingSearchTool } from '../bing-search-tool';
...
describe('BingSearchTool', () => {
  ...
  beforeEach(() => {
    ...
    tool = new BingSearchTool();
    tool.register({ server, sapiClient, auth });
  });

  it('registers a tool named "bing_search"', () => {
    expect(server.registerTool).toHaveBeenCalledWith(
      'bing_search',
      expect.objectContaining({ description: expect.stringContaining('Bing') }),
      expect.any(Function)
    );
  });

  it('passes bing_search target and parse: true to the scraper', async () => {
