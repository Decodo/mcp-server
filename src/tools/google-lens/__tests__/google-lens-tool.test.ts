...
import { GoogleLensTool } from '../google-lens-tool';
...
describe('GoogleLensTool', () => {
  ...
  beforeEach(() => {
    ...
    tool = new GoogleLensTool();
    tool.register({ server, sapiClient, auth });
  });

  it('registers a tool named "google_lens"', () => {
    expect(server.registerTool).toHaveBeenCalledWith(
      'google_lens',
      expect.objectContaining({ description: expect.stringContaining('Google Lens') }),
      expect.any(Function)
    );
  });

  it('passes google_lens target and parse: true to the scraper', async () => {
    sapiClient.scrape = jest.fn().mockResolvedValue({ data: {} });

    await registeredHandler({ query: 'https://example.com/image.jpg' });
  ...
  it('propagates scraper errors', async () => {
    ...
    await expect(registeredHandler({ query: 'https://example.com/image.jpg' })).rejects.toThrow(
